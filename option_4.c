...
int			find_best_ways_4(t_lem *lem)
{
	...
	lem->chosen_ways4 = (t_linklist**)malloc(500 * sizeof(t_linklist));
	lem->ways = 0;
	shortest = choose_first_room_4(lem, 0);
	while (shortest != NULL)
	{
		mem_shortest = shortest->room->cross;
		lem->waylist = NULL;
		add_room_to_waylist(lem, lem->startroom);
		create_way_4(shortest, shortest, lem, 1);
		if (lem->waylist->size < 100)
		{
			lem->chosen_ways4[lem->ways] = lem->waylist;
			lem->ways++;
		}
		else
		...
		shortest = choose_first_room_4(lem, mem_shortest);
	}
	arrange_ways(lem, lem->chosen_ways4);
	return (0);
}
