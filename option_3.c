...
int			create_way_3(t_linklist *link, t_linklist *c, t_lem *lem, int size)
{
	...
	while (t)
	{
		...
		== 0 && t->room->value <= b->room->value && t->room->value
		!= -1 && t->room->cross < b->room->cross))
			b = t;
		if (b->room == lem->endroom)
			break ;
		t = t->next;
	}
	create_way_3(b, c, lem, size + 1);
	return (0);
}
...
t_linklist	*choose_first_room_3(t_lem *lem, int cur_shortest)
{
	...
	while (link && link->room != lem->endroom && (link->room->cross <
	cur_shortest || link->room->used == 1 || link->room->value == -1))
		link = link->next;
	if (link == NULL)
		return (link);
	shortest = link;
	...
	while (link)
	{
		...
		link->room->value != -1 && link->room->cross <=
		shortest->room->cross))
			shortest = link;
		link = link->next;
	}
...
int			find_best_ways_3(t_lem *lem)
{
	...
	lem->chosen_ways3 = (t_linklist**)malloc(500 * sizeof(t_linklist));
	lem->ways = 0;
	shortest = choose_first_room_3(lem, 0);
	while (shortest != NULL)
	{
		mem_shortest = shortest->room->cross;
		lem->waylist = NULL;
		add_room_to_waylist(lem, lem->startroom);
		create_way_3(shortest, shortest, lem, 1);
		if (lem->waylist->size < 80)
		{
			lem->chosen_ways3[lem->ways] = lem->waylist;
			lem->ways++;
		}
		else
		...
		shortest = choose_first_room_3(lem, mem_shortest);
	}
	arrange_ways(lem, lem->chosen_ways3);
	return (0);
}
